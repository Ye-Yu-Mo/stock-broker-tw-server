...
## 验收标准
- [x] 服务启动后 `GET /health` 返回正常状态。
- [ ] 通过 API 可以登录/登出，状态接口能反映当前连接状态（FakeAdapter 已覆盖，真实 UAT 待开通后验证）。
- [x] 日志与审计中能看到登录请求和结果，密码不落明文。
- [x] `GET /metrics` 能输出 Prometheus 文本格式。
- [x] 未带 token 的 API 请求返回 401。
- [x] M1 的单元测试全部通过。
