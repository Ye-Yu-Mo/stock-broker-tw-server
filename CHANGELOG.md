...
- 启动恢复现在识别已过交易日且明确为非预约的股票委托，并将其归档为本地 `FAILED` 终态。
- 过期归档保留原交易日和 `recovery_reason`，不伪造券商撤单，不影响预约单、当天订单、字段不足订单或查询失败场景。
- 恢复摘要新增 `expired_orders` 计数，避免历史跨日委托长期伪装成当前未完成挂单。

### Verification

- 本地自动化测试：350 passed；Ruff 和 diff 检查通过。
- PROD `J01ki` 已确认属于前一交易日非预约委托；本补丁待部署后验证本地恢复归档结果。
...
- 收紧 `client_order_id` 幂等冲突判断，避免并发请求使用不同订单参数时错误复用已有订单。
- 改善 SQLite 订单状态更新的一致性，保留并发写入的响应、报告和状态转换历史，并增加写锁等待时间。
- 隔离单条异常券商回报，避免关联撤单/改单状态异常中断后续事件消费。
- 串行化同一应用内的恢复流程与行情订阅操作，避免重复恢复、订阅数量超限和重复调用 SDK。
- 将同步行情订阅 SDK 调用移出 asyncio 事件循环，避免阻塞 HTTP/WebSocket 服务。

### Verification

- 本地自动化测试：346 passed；Ruff 和 diff 检查通过。
- 未执行真实 PROD 写操作；真实 SDK 联调继续按 UAT/PROD 清单单独验证。
...
- 修复元大 SDK 对价格 `double` 内部截断导致合法的 `45.48` 被识别为 `45.4799`，增加 .NET 边界的最小 ULP 价格修正。

### Verification

- 本地自动化测试：339 passed；Ruff、锁文件检查和 diff 检查通过。
- 远程 PROD 已确认原始问题为券商拒绝价格精度，不存在成交。
...
- 语义化 `ap_code`：`REGULAR`、`ODD_LOT`、`INTRADAY_ODD_LOT`、`AFTER_HOURS`，兼容旧数字值 `0/2/4/7`。
- 真实 Spark 登录时序等待交易主机连接，并增加脱敏系统事件诊断。
- 增加真实订单同步响应超时后的不确定状态恢复与 BasketNo 对账。

### Fixed

- 修复真实 Spark `List[StockOrder]`、`enumLangType` 和登录连接时序问题。
- 修复同步交易响应超时但券商订单实际成立时，本地订单被错误标记为 `FAILED` 的问题。
- 修复收盘后/异步回报场景下的订单状态与本地状态不一致问题。

### Verification

- 自动化测试：334 passed；Ruff、锁文件检查和 diff 检查通过。
- PROD 已完成登录、资金、持仓、委托/成交回报和恢复验证；未宣称完整 PROD 交易验收。
