...
## 验收标准
- [ ] 测试环境能成功下一笔股票买单（待 UAT 开通后验证）。
- [x] 使用 `client_order_id` 重复提交同一请求不会重复下单（单测 + API 集成测试已覆盖）。
- [x] 能对未成交委托执行撤单，状态变为已撤（撤单参数单测 + RR_RealReport 取消回报单测已覆盖，真实 UAT 待验证）。
- [x] 风控拒绝时返回明确错误，并写入审计（risk 单测 + API 400 测试已覆盖）。
- [x] 回报到达后订单状态能自动更新并通过 WebSocket 推送（report handler + WS 集成测试已覆盖）。
- [x] M1、M2、M3 的单元测试全部通过。
